...
--Cau 1. Liệt kê danh sách khách hàng ở Đà Nẵng
Select KH.Cust_id, Cust_ad
From customer KH
...
SELECT Cust_name,Cust_id,Cust_phone
From customer 
Except 
SELECT Cust_name,Cust_id,Cust_phone
From customer
where Cust_phone like '090%' 
...
select Cust_id, Cust_name
from customer 
where Cust_name like N'_h%' or Cust_name like N'_t%' or Cust_name like N'_a%' or Cust_name like N'_ê%'

select Cust_id, Cust_name
from customer 
where Cust_name like N'_[h,t,ê,a,á,ã,à,ạ,ế,ề,ễ,ể,ệ]%'
--where SUBSTRING(Cust_name, LEN(Cust_name) -1, 0) IN ('h', 't', 'a', N'ê');
--7.	Liệt kê những giao dịch diễn ra trong quý IV năm 2016
select t_id, t_date
from transactions
where year(t_date)=2016 and (month (t_date) =10 or month (t_date)=11 or month (t_date)=12)
...
select t_id, t_date
from transactions
where year(t_date)=2016 and (month (t_date) =7 or month (t_date)=8 or month (t_date)=9)

select t_id, t_date
from transactions
where year(t_date)= 2016
and month(t_date) in (7,8,9)
--9.	Liệt kê những khách hàng không thuộc các chi nhánh miền bắc
SELECT Cust_id, Br_id
FROM customer
...
select *
from transactions
where t_type = '1' and (t_time < '08:00' or t_time>'17:00')

select t_id, t_amount, t_date, t_time ,t_type
from transactions
where t_type = '1' and t_time not between '07:30' and '11:30'  and t_time not between '13:00' and '16:30'
...
SELECT *
FROM Branch
where BR_ad like '' or BR_ad is null
...
Select Cust_name N'Tên khách hàng', Cust_ad  N' Thành phố'
From customer

SELECT c.Cust_name, b.BR_name
FROM customer c
INNER JOIN branch b ON c.Br_id = b.BR_id;
...
select *
from customer
where Cust_name like '%___a' or Cust_name like '%___u' or Cust_name like '%___i'
...
SELECT *
FROM customer
WHERE SUBSTRING(Cust_name, LEN(Cust_name) - 1, 1) IN ('u', N'ũ', 'a');

select Cust_name,BR_id,
right ( Cust_name, charindex(' ',REVERSE(Cust_name)) -1) as Ten
from customer
where right (Cust_name, charindex(' ',reverse(Cust_name))-1) like N'_[u,ũ,ú,ù,ụ,a,á,à,ã,ả,ạ,ủ]%'
--18.	Hiển thị tên khách hàng và tên tỉnh/thành phố mà họ sống
select Cust_name,Cust_ad
	right (Cust_name, charindex(' ',REVERSE(Cust_name)) -1) as Ten,
	right( cust_ad, -1 + case charindex('-', reverse(cust_ad))
						when 0 then charindexx (',', reverse(
from customer